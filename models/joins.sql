with prod as (
    ...
    select
        ...
        ,od.discount
    from 
        {{ref("order_details")}} od
        left join
        prod pd on (od.product_id = pd.product_id)
), ordrs as (
    select
        ord.order_date
        ,ord.order_id
    ...
    from
        ...
        left join
        {{ref("customers")}} cs on (ord.customer_id = cs.customer_id) 
        left join
        {{ref("employees")}} em on (ord.employee_id = em.employee_id)
        left join
        {{source("sources", "shippers")}} sh on (ord.ship_via = sh.shipper_id)
), final_join as (
    select
        od.*
        ,ord.order_date
    ...
    from
        orddetai od
        inner join
        ordrs ord on (od.order_id = ord.order_id)
)
select 
    * 
from 
